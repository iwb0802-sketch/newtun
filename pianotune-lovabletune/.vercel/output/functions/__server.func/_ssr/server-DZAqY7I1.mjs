...
async function getStartManifest(matchedRoutes) {
  const { tsrStartManifest } = await import("../_tanstack-start-manifest_v-BFSO9u8E.mjs");
...
function attachRouterServerSsrUtils({ router, manifest: manifest2, getRequestAssets }) {
  ...
  const serverSsr = {
    ...
    dehydrate: async (opts) => {
      ...
      if (manifest2) {
        getMatchedRoutesCacheKey(matchesToDehydrate);
        const preparedManifest = getPreparedMatchedManifestRoutes(manifest2, matchesToDehydrate);
        manifestToDehydrate = {
          ...manifest2.scriptFormat ? { scriptFormat: manifest2.scriptFormat } : {},
...
async function loadEntries() {
  const [routerEntry, startEntry, pluginAdapters] = await Promise.all([
    import("./router-DcjR50EF.mjs"),
    import("./start-D7m28w6T.mjs"),
